...
open class TenClock : UIControl{
    ...
    @IBInspectable var pathWidth:CGFloat = 54
    ...
    var headAngle: CGFloat = 0{
        didSet{
            if (headAngle > fourPi  +  CGFloat(Double.pi/2.0)){
                headAngle -= fourPi
            }
    ...
    //Happy Waves changes
    open var headImageName: String?
    open var tailImageName: String?
    ...
    var strokeColor: UIColor {
        ...
        set(strokeColor) {
            //Happy Waves Changes
            if disabled {
                trackLayer.strokeColor = strokeColor.withAlphaComponent(0.1).cgColor //- orig code
            } else {
                trackLayer.strokeColor = UIColor(red: 253.0/255.0, green: 234.0/255.0, blue: 126.0/255.0, alpha: 1.0).cgColor
            }
    ...
    open func update() {
        ...
        //updateGradientLayer() //Happy Waves Changes - remove gradient
    ...
    func updateHeadTailLayers() {
        ...
        headLayer.fillColor = UIColor(red: 95.0/255.0, green: 224.0/255.0, blue: 209.0/255.0, alpha: 1.0).cgColor
        tailLayer.fillColor = UIColor(red: 95.0/255.0, green: 224.0/255.0, blue: 209.0/255.0, alpha: 1.0).cgColor
        ...
        if let tImageName   = self.tailImageName {
            
            topTailLayer.contents   = UIImage(named:tImageName)?.cgImage
            topTailLayer.fillColor  = disabledFormattedColor(UIColor.clear).cgColor
            
        } else {
            
            topTailLayer.contents   = nil
            topTailLayer.fillColor  = disabledFormattedColor(self.tintColor.greyscale).cgColor
            tailLayer.fillColor     = self.tintColor.greyscale.cgColor
            let stText = tlabel("", color: disabledFormattedColor(headTextColor))
            stText.position = topTailLayer.center
            topTailLayer.addSublayer(stText)
            
        }
        
        if let hImageName = self.headImageName {
            
            topHeadLayer.contents   = UIImage(named:hImageName)?.cgImage
            topHeadLayer.fillColor  = disabledFormattedColor(UIColor.clear).cgColor
            
        } else {
            
            topHeadLayer.contents   = nil
            topHeadLayer.fillColor  = disabledFormattedColor(self.tintColor.greyscale).cgColor
            headLayer.fillColor     = self.tintColor.greyscale.cgColor
            let endText = tlabel("", color: disabledFormattedColor(tailTextColor))
            endText.position = topHeadLayer.center
            topHeadLayer.addSublayer(endText)
